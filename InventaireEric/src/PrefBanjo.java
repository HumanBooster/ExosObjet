...
public class PrefBanjo extends PrefInstrument {
    @objid ("8d11616e-5c6b-423b-98b9-11a2932ac1b1")
    private int nbCordes;

    @objid ("0e4d9b46-86be-48a8-bf87-b85e79bd3ffb")
    public PrefBanjo(Fabricant fabricant, String modele, Type type, Bois boisFond, Bois boisTable, int nbCordes) {
        super(fabricant, modele, type, boisFond, boisTable);
        this.nbCordes = nbCordes;
    }

    @objid ("f98d24fa-e4fc-4fe3-915b-dcf790f43fe1")
    public boolean equals(PrefInstrument prefBanjo) {
        System.out.println("On compare le nombre de cordes...");
        if (this == prefBanjo) {
            return true;
        }
        
        if (!(prefBanjo instanceof PrefBanjo))
        	return false;
        
        if (!super.equals(prefBanjo))
        	return false;
        
        PrefBanjo pref = (PrefBanjo) prefBanjo;
